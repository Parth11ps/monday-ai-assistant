...
CHANNELS = 1
SEND_SAMPLE_RATE = 16000 # Gemini's recommended sample rate for audio input
RECEIVE_SAMPLE_RATE = 24000 # ElevenLabs' sample rate for TTS output
CHUNK_SIZE = 1024 # Number of audio frames per buffer read/write
MODEL = "gemini-2.5-flash" # Gemini model to use
VOICE_ID = 'pFZP5JQG7iQjIQuC4Bku'
DEFAULT_MODE = "none"  
MAX_OUTPUT_TOKENS = 100 # Limit for Gemini's response length to keep interactions snappy and focused
...
class AIAnimationWidget(QWidget): 
    ...
    def paintEvent(self, event):
        ...
        for x, y, point_size, alpha in projected_points:
            color = QColor(255, 120, 120, alpha) if self.is_speaking else QColor(255, 45, 45, alpha)
...
class AI_Core(QObject):
    """
     Core backend engine implementing a hybrid AI architecture
     Combines:
      Gemini LLM for reasoning
      Multimodal input (text + webcam/screen)
     Local system tools (files, apps, browser)
    """
    ...
    activity_received = Signal(str, str)
    ...
    def __init__(self, video_mode=DEFAULT_MODE):
        ...
        self.config = {
            ...
            "system_instruction": """
            Your name is Monday and you are my personal AI productivity assistant.
    ...
    def _resolve_directory_path(self, location_text):
        # Normalizes natural-language directory references into real filesystem paths
        # This lets commands like "desktop" or "documents" map to platform paths
        if not location_text:
            return '.'
        location = location_text.strip().strip('"').strip("'")
        location_lower = location.lower()
        if location_lower in {"here", "current directory", "current folder", "."}:
            return '.'
        if "desktop" in location_lower:
            return os.path.expanduser("~/Desktop")
        if "downloads" in location_lower:
            return os.path.expanduser("~/Downloads")
        if "documents" in location_lower or "document" in location_lower:
            return os.path.expanduser("~/Documents")
        if location_lower in {"home", "home directory", "~"}:
            return os.path.expanduser("~")
        return os.path.expanduser(location)

    def _handle_local_tool_request(self, user_text): # Parses user text for local tool commands and executes them directly, returning True if a command was handled
        text = user_text.strip()

        # Detect website-opening requests and execute them locally
        website_match = re.search(r"(?:open|launch)\s+(?:the\s+)?(?:website|site|url)\s+(.+)$", text, re.IGNORECASE)
        if website_match:
            url = website_match.group(1).strip().strip('"').strip("'")
            result = self._open_website(url)
            html = f"<p><strong>WEBSITE</strong></p><p>{escape(result.get('message', 'No message returned.'))}</p>"
            self.activity_received.emit("WEBSITE", html)
            return True, result.get("message", "Website request completed.")

        # Detect desktop application launch requests
        app_match = re.search(r"(?:open|launch|start)\s+(?:the\s+)?(?:application\s+|app\s+)?(.+)$", text, re.IGNORECASE)
        if app_match and not re.search(r"(?:website|site|url)", text, re.IGNORECASE):
            application_name = app_match.group(1).strip().strip('"').strip("'")
            result = self._open_application(application_name)
            html = f"<p><strong>APPLICATION</strong></p><p>{escape(result.get('message', 'No message returned.'))}</p>"
            self.activity_received.emit("APPLICATION", html)
            return True, result.get("message", "Application request completed.")

        # Detect folder-creation commands and optionally resolve a target location
        folder_match = re.search(r"create\s+(?:a\s+)?folder\s+named\s+(.+?)(?:\s+in\s+(?:the\s+)?(.+))?$", text, re.IGNORECASE)
        if folder_match:
            folder_name = folder_match.group(1).strip().strip('"').strip("'")
            location_text = folder_match.group(2)
            base_path = self._resolve_directory_path(location_text) if location_text else '.'
            folder_path = os.path.join(base_path, folder_name)
            result = self._create_folder(folder_path)
            html = f"<p><strong>CREATE FOLDER</strong></p><p>{escape(result.get('message', 'No message returned.'))}</p><p><strong>Path:</strong> {escape(folder_path)}</p>"
            self.activity_received.emit("FILESYS", html)
            return True, result.get("message", "Folder request completed.")

        # Detect file-creation commands, including optional location and content
        file_match = re.search(r"create\s+(?:a\s+)?file\s+named\s+(.+?)(?:\s+in\s+(?:the\s+)?(.+?))?(?:\s+with\s+content\s+(.+))?$", text, re.IGNORECASE)
        if file_match:
            file_name = file_match.group(1).strip().strip('"').strip("'")
            location_text = file_match.group(2)
            content_text = file_match.group(3) or ""
            base_path = self._resolve_directory_path(location_text) if location_text else '.'
            file_path = os.path.join(base_path, file_name)
            result = self._create_file(file_path, content_text)
            html = f"<p><strong>CREATE FILE</strong></p><p>{escape(result.get('message', 'No message returned.'))}</p><p><strong>Path:</strong> {escape(file_path)}</p>"
            self.activity_received.emit("FILESYS", html)
            return True, result.get("message", "File request completed.")

        # Detect directory listing requests
        list_match = re.search(r"(?:list|show)\s+(?:all\s+)?(?:files|items)(?:\s+in\s+(.+))?$", text, re.IGNORECASE)
        if list_match:
            directory_text = list_match.group(1)
            directory_path = self._resolve_directory_path(directory_text) if directory_text else '.'
            result = self._list_files(directory_path)
            if result.get("status") == "success":
                self.file_list_received.emit(result.get("directory_path", directory_path), result.get("files", []))
            else:
                html = f"<p><strong>FILESYS</strong></p><p>{escape(result.get('message', 'No message returned.'))}</p>"
                self.activity_received.emit("FILESYS", html)
            return True, result.get("message", "List files request completed.")

        # Detect requests to read and display file contents
        read_match = re.search(r"(?:read|show)\s+(?:the\s+)?file\s+(.+)$", text, re.IGNORECASE)
        if read_match:
            file_path = os.path.expanduser(read_match.group(1).strip().strip('"').strip("'"))
            result = self._read_file(file_path)
            if result.get("status") == "success":
                content = result.get("content", "")
                html = f"<p><strong>READ FILE</strong></p><p><strong>Path:</strong> {escape(file_path)}</p><pre style='white-space: pre-wrap; word-wrap: break-word; color: #f4dede; font-size: 9pt;'>{escape(content)}</pre>"
                self.activity_received.emit("FILESYS", html)
                return True, f"I successfully read the file '{file_path}'."
            html = f"<p><strong>READ FILE</strong></p><p>{escape(result.get('message', 'No message returned.'))}</p>"
            self.activity_received.emit("FILESYS", html)
            return True, result.get("message", "Read file request completed.")
    ...
    async def stream_video_to_gui(self): # Continuously captures frames from the selected video source and emits them to the GUI
        video_capture = None
            ...
            try:
                if self.video_mode == "camera":
                    ...
                    if video_capture is not None:
                        ...
                        video_capture = None
                    screenshot = await asyncio.to_thread(ImageGrab.grab) # Capture the entire screen; for multi-monitor setups, this will include all monitors in a single wide image
    ...
    async def send_frames_to_gemini(self):
        # Prepares compressed image frames for Gemini-compatible transmission
        # This supports a separate realtime multimodal path if needed
        while self.is_running:
            await asyncio.sleep(1.0)
            if self.video_mode != "none" and self.latest_frame is not None: # Only send frames if a video mode is active and we have a frame to send
    ...
    async def receive_text(self):
        # Handles streamed model responses, tool calls, grounding links, and code outputs
        # Also routes tool results back into the UI
        while self.is_running:
            try:
                turn_urls, turn_code_content, turn_code_result, file_list_data = set(), "", "", None
                turn = self.session.receive()
                async for chunk in turn:
                    # Execute model-requested tool calls and send structured results back
    ...
    async def process_text_input_queue(self):
        while self.is_running:
            ...
            for q in [self.response_queue_tts, self.audio_in_queue_player]:
                while not q.empty():
                    q.get_nowait()
            await self.generate_text_response(text)
            self.text_input_queue.task_done()

    async def generate_text_response(self, user_text): 
         # Main AI decision pipeline:
         # Handles user input by deciding between:
         # - Local tool execution (fast, system-level actions)
         # - Gemini LLM (reasoning, general queries, multimodal understanding)
        try:
            handled, local_response = self._handle_local_tool_request(user_text)
            if handled:
                response_text = local_response or "The requested action has been completed."
            else:
                prompt = f"System instructions:\n{self.config['system_instruction']}\n\nUser: {user_text}\nAssistant:"
                
                contents = [prompt]

                # Attach latest frame for multimodal input if available
                if self.video_mode != "none" and self.latest_frame is not None:
                    frame_rgb = cv2.cvtColor(self.latest_frame, cv2.COLOR_BGR2RGB)
                    pil_img = PIL.Image.fromarray(frame_rgb)
                    contents.append(pil_img) # The Gemini API can handle PIL images directly in the contents list, and will treat them as multimodal input to reason about when generating a response.
            ...
            await self.response_queue_tts.put(response_text)
            self.end_of_turn.emit()
            await self.response_queue_tts.put(None)
        except Exception as e:
            error_text = str(e)
            if "quota" in error_text.lower() or "1011" in error_text:
                friendly_message = "I could not respond because the Gemini API quota or service limit was reached. Please try again later or use a different API key/project."
            else:
                friendly_message = f"I ran into an error while generating a response: {error_text}"
            self.text_received.emit(friendly_message)
            await self.response_queue_tts.put(friendly_message)
            self.end_of_turn.emit()
            await self.response_queue_tts.put(None)

    async def tts(self):
        # Streams generated text to ElevenLabs and converts it into PCM audio
        # Audio chunks are pushed into the playback queue for real-time speech output
    ...
    async def play_audio(self):
        # Consumes decoded audio bytes and plays them through the local audio output stream
    ...
    async def main_task_runner(self):
        # Starts the main concurrent backend tasks for video, response generation, TTS, and playback
        self.tasks.extend([
    ...
    def start_event_loop(self):
        # Binds the backend asyncio loop to this thread and starts execution
    ...
    def stop(self):
        # Gracefully shuts down async tasks and closes active audio resources
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Monday - Personal AI Productivity Assistant")
        ...
        self.setStyleSheet("""
            ...
            QLineEdit#input_box:focus { border: 1px solid #ff3b3b; }
            QLabel#video_label { 
            ...
            QPushButton:hover { background-color: #7a1111; color: #ffffff; }
            QPushButton:pressed { background-color: #ff3b3b; color: #ffffff; border: 1px solid #ff3b3b; }
        ...
        self.is_first_monday_chunk = True
        self.current_video_mode = DEFAULT_MODE
        self.setup_backend_thread()

    def setup_backend_thread(self):
        # Creates the AI backend, connects GUI signals/slots, and launches the backend thread
        # This separates UI rendering from long-running AI and media tasks
        ...
        self.ai_core.activity_received.connect(self.update_activity)
    ...
    def send_user_text(self):
        # Collects typed input, displays it in the chat panel, and emits it to the backend
        text = self.input_box.text().strip()
        if text:
            self.text_display.append(f"<p style='color:#ff8a8a; font-weight:bold;'>&gt; USER:</p><p style='color:#f4dede; padding-left: 10px;'>{escape(text)}</p>")
            self.is_first_monday_chunk = True
    ...
    def update_text(self, text):
        if self.is_first_monday_chunk:
            self.is_first_monday_chunk = False
            self.text_display.append(f"<p style='color:#ff5a5a; font-weight:bold;'>&gt; MONDAY:</p>")
    ...
    def update_search_results(self, urls):
        ...
        for i, url in enumerate(urls):
            ...
            html_content += f'<p style="margin:0; padding: 4px;">{i+1}: <a href="{url}" style="color: #ff7a7a; text-decoration: none;">{display_text}</a></p>'
        self.tool_activity_display.setText(html_content)

    @Slot(str, str)
    def update_activity(self, section, html):
        base_title = "SYSTEM ACTIVITY"
        self.tool_activity_title.setText(f"{base_title} // {section}")
    ...
    def display_executed_code(self, code, result):
        ...
        if result:
            html += f'<p style="color:#ff5a5a; font-weight:bold; margin-top:10px; margin-bottom: 5px;">&gt; OUTPUT:</p><pre style="white-space: pre-wrap; word-wrap: break-word; color: #ffd6d6; font-size: 9pt;">{escape(result.strip())}</pre>'
        self.tool_activity_display.setText(html)
    ...
    def update_file_list(self, directory_path, files):
        ...
        html = f'<p style="color:#ff5a5a; margin-bottom: 5px;">DIR &gt; <strong>{escape(directory_path)}</strong></p>'
        if not files:
            ...
            for folder in folders: html += f'<li style="margin: 2px 0; color: #ff9c9c;">[+] {escape(folder)}</li>'
            for file_item in file_items: html += f'<li style="margin: 2px 0; color: #f4dede;">&#9679; {escape(file_item)}</li>'
            html += '</ul>'
        self.tool_activity_display.setText(html)

    @Slot()
    def add_newline(self):
        if not self.is_first_monday_chunk: self.text_display.append("")
        self.is_first_monday_chunk = True

    @Slot(QImage)
    def update_frame(self, image):
        # Renders the latest webcam or screen frame inside the right-hand preview panel
